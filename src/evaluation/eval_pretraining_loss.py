...
def get_pretraining_data(tokenizer, max_seq_len, n_tkns, save_path):
    ...
    ds = ds.map(
        lambda x: {"text": tokenizer.batch_decode(x["input_ids"])}, batched=True
    )
    print(ds)
    print(ds["text"][0])
    # Save the dataset
...
def get_path_to_pretraining_data(model_name, max_seq_len, save_dir):
    save_path = os.path.join(
        save_dir, f"{model_name}", f"max_seq_len_{max_seq_len}", "tokenized_samples.hf"
    )
    return save_path
...
def main():
    ...
    parser.add_argument(
        "--data_save_dir",
        type=str,
        default="data/processed_data/pretraining_data/the_pile",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--results_save_path",
        type=str,
        default="results/eval_settings/pretrain_eval",
    ...
    parser.add_argument(
        ...
        help="Directory to save base model results",
    )
    args = parser.parse_args()

    models_to_eval = pd.read_csv(args.model_selection_path)
    seeds = args.seeds
    pretrain_eval_path = args.results_save_path
    base_model_results_path = args.results_save_path_base
    ...
    for _, row in models_to_eval.iterrows():
        ...
        data_path = get_path_to_pretraining_data(
            model_size, max_seq_len, args.data_save_dir
        )
        if not os.path.exists(data_path):
            _, tokenizer = load_model(base_model_name, lora_adapter_path=None)
            ds = get_pretraining_data(
                tokenizer, max_seq_len, n_tkns=args.n_tkns_to_save, save_path=data_path
            )
        else:
        ...
        ft = row["ft"]
        # lora_module = row["lora_module"]
        lora_module = "attn_only" if ft == "lora" else ""
        lora_rank = row["lora_rank"]

        losses = {}

        # Check if base model eval is already done: this is done to avoid re-evaluating the base model
        ...
        consolidated_output_dir_root = os.path.join(
            pretrain_eval_path,
            model_size,
            domain,
            dataset_name,
        ...
        ft_key = (
            f"{ft}_{lora_module}_r{int(lora_rank)}_ppl" if ft != "full" else f"{ft}_ppl"
        )

        for seed in seeds:
            output_path = os.path.join(
                consolidated_output_dir_root, f"seed_{seed}", "losses.json"
            )
            loss = None
        ...
        print("Losses:\n", losses)
        print(f"\nSaved losses to {consolidated_output_dir_root}")
