...
class TokenGenerator:
    def __init__(
        ...
        self.model, self.tokenizer = load_model(
            model_name=self.model_name, lora_adapter_path=self.lora_adapter_path
        )
    ...
    def inference_step(
        ...
        for i in range(logits.size(1) - 1):
            ...
            entropy = -torch.sum(
                next_token_probs_dist * torch.log(next_token_probs_dist)
            )
            actual_next_token_id = input_id[0, i + 1]
            actual_next_token_prob = next_token_probs_dist[
                0, actual_next_token_id
            ].item()
            ...
            actual_next_token_rank = (
                torch.where(indices == actual_next_token_id)[0][0].item() + 1
            )
            ...
            top_k_token_probs = torch.round(
                top_k_token_preds.values, decimals=3
            ).reshape(1, -1)
        ...
        return {
            ...
            "entropies": entropies,
            "output_dist": next_token_probs_dist[0],
        }

    def generate_tokens(self, input_ids: torch.Tensor) -> tuple[list, torch.Tensor]:
        """
        Get next token probabilities for a given model and input ids
        """
        ...
        for instance_num, input_id in enumerate(input_ids):
            ...
            with torch.no_grad():
                ...
                next_tkn_outputs = TokenGenerator.inference_step(
                    logits=logits, input_id=input_id, device=device, top_k=self.top_k
                )
            ...
            output_dist = next_tkn_outputs["output_dist"]
            ...
            in_token_ids = [
                input_id[:, :-1][0][max(0, i - 128) : i + 1].cpu().numpy()
                for i in range(len(input_id[:, :-1][0]))
            ]
            ...
            vocab_dist = torch.cat((vocab_dist, output_dist))

        return output_tkns, vocab_dist

    def get_reshaped_input_ids(ds: Dataset, device: str) -> torch.Tensor:
        """
        ...
        input_ids = torch.reshape(
            input_ids, (input_ids.shape[0], 1, input_ids.shape[1])
        )
        return input_ids

    def iterate_over_ds(self, ds: Dataset) -> tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        model_outputs, vocab_dist = self.generate_tokens(input_ids)
        return model_outputs, vocab_dist
