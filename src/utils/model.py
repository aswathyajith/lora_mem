...
def load_model(model_name: str, lora_adapter_path: str, merge_and_unload: bool = False):
    """
...
def prompt_model(
    ...
    if not tokenized:
        full_inp_ids = (
            ...
            .to("cuda")
            .input_ids
        )  # encode batch of input seq
    else:
        full_inp_ids = torch.Tensor(batch["input_ids"]).to("cuda")
    # print(full_inp_ids)
    input_ids = full_inp_ids[
        :, :prompt_context_len
    ]  # get the input tokens that we want to prompt model with
    inp_prompt = tokenizer.batch_decode(input_ids)
    model_comp_ids = model.generate(
        input_ids, max_length=128, early_stopping=True, do_sample=False
    )
