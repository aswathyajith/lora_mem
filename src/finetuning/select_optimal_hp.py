...
def match_data_args(
    ...
    if not os.path.exists(preprocess_config_path):
        raise FileNotFoundError(
            f"Preprocess config file {preprocess_config_path} does not exist"
        )
...
def get_eval_loss(
    ...
    model_dir = (
        lora_adapter_path if lora_adapter_path is not None else model_path_or_hf_name
    )
    ...
    if os.path.exists(trainer_state_results):
        ...
        print(
            f"No trainer_state.json found in {model_dir}. Loading final model and evaluating on test set."
        )
        ...
        dataset = load_data(tokenizer, **data_args)
        ...
        loss = compute_loss(
            model_path_or_hf_name=model_path_or_hf_name,
...
def get_avg_loss(
    ...
    for i in seeds:
        if lora_adapter_path is not None:
            ...
            _lora_adapter_path = os.path.join(
                lora_adapter_path, f"seed_{i}", "final_model"
            )
        else:
            _model_name_or_path = os.path.join(
                model_name_or_path, f"seed_{i}", "final_model"
            )
...
def main():
    ...
    # Example Usage:
    # python src/find_opt_lr.py --all_models_path models/pythia-1.4b/packing_n_docs/perturbations/none --save_path configs/optimal_lr_n_docs.json --max_seq_lens 64 128 256 --save_path config_dfs/optimal_hp.json
    parser.add_argument(
        "--all_models_path",
        type=str,
    ...
    for k, v in dict_losses.items():
        ...
        max_len = [
            substr.replace("max_seq_len_", "")
            for substr in k.split("/")
            if substr.startswith("max_seq_len")
        ][0]
        n_tkns = [
            n.replace("n_tkns_", "") for n in k.split("/") if n.startswith("n_tkns_")
        ][0]
    ...
    for model, hyperparams in path_hyperparams.items():
        ...
        opt_lr, path_loss = min(
            hyperparams["lr_loss_map"].items(), key=lambda x: x[1][1]
        )
