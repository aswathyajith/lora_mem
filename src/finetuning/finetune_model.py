...
from src.utils.data import load_data, set_seed
...
class SaveBestModelCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics, **kwargs):
        ...
        if eval_loss is None:
            print("Warning: eval_loss not found in metrics")
            return

        print(
            f"\nCurrent eval_loss: {eval_loss}, Best eval_loss so far: {self.best_eval_loss}"
        )

        if eval_loss < self.best_eval_loss:
            self.best_eval_loss = eval_loss
            print(f"\nNew best eval_loss: {eval_loss}. Saving model...")
            ...
            try:
                ...
                kwargs["tokenizer"].save_pretrained(
                    os.path.join(args.output_dir, "final_model")
                )

                # Save the evaluation metrics
                with open(
                    os.path.join(args.output_dir, "final_model", "trainer_state.json"),
                    "w",
                ) as f:
                    json.dump(metrics, f, indent=4)

                print("Successfully saved best model and metrics")
...
class Finetuning:
    ...
    def load_configs(self, data_config, model_config_path, lora_config_path=None):
        """
        Loads the model config, lora config, and data config. This needs to be done first to set the output directory.
        """
        seed = self.seed
        ...
        if self.lora and (lora_config_path is not None):
            with open(lora_config_path, "r") as f:
                ...
                # all-linear or attn_only target_modules
                if self.target_modules is None or self.target_modules == "attn_only":
                    lora_config["target_modules"] = ["query_key_value"]
                else:
                    lora_config["target_modules"] = self.target_modules

                # set "target_modules" directory
                if isinstance(self.target_modules, str):
                    lora_module_dir = self.target_modules
                elif self.target_modules is None:
                    lora_module_dir = "attn_only"  # default

                lora_rank = lora_config["r"]
                full_lora = os.path.join("lora", lora_module_dir, f"r_{lora_rank}")
        ...
        with open(model_config_path, "r") as f:
            ...
            if n_train_tkns is not None:
                print(n_train_tkns)
                n_train_str = "n_tkns_" + "{:.0e}".format(n_train_tkns).replace(
                    "e+0", "e"
                )
            else:
                n_train_str = "num_train_" + num_train if num_train != -1 else "all"
            lr_str = "lr_" + "{:.0e}".format(lr).replace(
                "e-0", "e-"
    ...
    def remove_stopwords(sample, tokenizer, text_field):
        ...
        for s in sample:
            s[text_field] = " ".join(
                [word for word in s[text_field].split() if word not in sw]
            )
        ...
        for perturbation in perturbations:
            if perturbation == "reverse_tkns":
                self.train_dataset = self.train_dataset.map(
                    self.reverse_tkns,
                    fn_kwargs={"text_field": self.data_config["text_field"]},
                )
    ...
    def finetune_model(
        self, sft_config_path, field="text", resume_from_checkpoint=False
    ):
        ...
        try:
            ...
            print(
                f"Training failed with resume_from_checkpoint={resume_from_checkpoint}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_config_path",
        default="configs/model_configs/pythia-160m.json",
        type=str,
    )
    parser.add_argument(
        "--sft_config_path", default="configs/model_configs/sft_configs.json", type=str
    )
    parser.add_argument(
        "--lora_config_path", default="configs/lora_config.json", type=str
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Target modules for Lora finetuning",
    )
    parser.add_argument(
        "--domain", default="wiki", type=str, help="Domain of the dataset"
    )
    parser.add_argument(
        "--dataset_config",
    ...
    parser.add_argument(
        ...
        help="If this flag is set, Lora finetuning will be done (full parameter finetuning is the default)",
        default=False,
    )
    parser.add_argument(
        "--num_train", default=None, type=int, help="Number of training samples"
    )
    parser.add_argument(
        "--n_train_tkns", default=None, type=float, help="Training token budget"
    )
    parser.add_argument(
        "--resume_from_checkpoint",
        action=argparse.BooleanOptionalAction,
        help="If this flag is set, finetuning will resume from the last checkpoint",
        default=False,
    )
    parser.add_argument(
        "--pad_sequences",
        action=argparse.BooleanOptionalAction,
        help="If this flag is set, include sequences shorter than max_len by padding them (by default, this flag is not set, i.e., we only include consider instances with at least max_seq_len tokens)",
    )
    parser.add_argument(
        "--packing",
        default=False,
        action="store_true",
        help="If this flag is set, pack sequences to max_len (pad_sequences will be ignored if this flag is set)",
    )
    parser.add_argument(
        "--stream_size",
        default=None,
        type=int,
        help="Number of training samples to stream if streaming is set to True",
    )
    parser.add_argument(
        "--perturbations",
        choices=[
    ...
    for dataset in datasets.keys():
        ...
        for max_seq_len in max_seq_lens:
            ...
            if os.path.exists(model_save_dir) and "final_model" in os.listdir(
                model_save_dir
            ):
            ...
            ft.init_dataset(
                hf_repo, pad_sequences=pad_sequences, max_length=max_seq_len
            )
            # print(f"FT dataset size: {len(ft.train_dataset)}")
            ft.perturb_dataset(perturbations)
            ft.finetune_model(
                sft_config_path,
                field=data_config["text_field"],
